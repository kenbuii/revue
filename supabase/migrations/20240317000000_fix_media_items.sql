...
BEGIN
    ...
    WHERE tc.table_name = 'posts'
        ...
        AND kcu.column_name = 'media_item_id';

    RAISE NOTICE 'Posts foreign key details: %', posts_fk_record;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added isbn column';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added total_ratings column';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added popularity_score column';
    END IF;
...
-- Update the upsert_media_item function
...
DECLARE
    media_item_record RECORD;
BEGIN
    -- Check if our specific media item exists after all changes
    SELECT id, title, media_type, created_at 
    INTO media_item_record
    FROM media_items 
    WHERE id = 'book_the_knight_and_the_moth_1749167489711';

    RAISE NOTICE 'Final media item check: %', media_item_record;
END $$; 